...
resource "aws_kms_key" "state" { description = "Creator Link Store Terraform state encryption"; enable_key_rotation = true }
...
resource "aws_dynamodb_table" "lock" { name = "creator-store-terraform-lock"; billing_mode = "PAY_PER_REQUEST"; hash_key = "LockID"; attribute { name = "LockID"; type = "S" } }
